...
// https://shwjdqls.github.io/android-get-distance-between-locations/
...
object DistanceUtil {
    ...
    fun getDistance(geo1: GeoPoint, geo2: GeoPoint): Double {
        ...
        val a =
            sin(dLat / 2).pow(2.0) + sin(dLon / 2).pow(2.0) * cos(Math.toRadians(geo1.latitude)) * cos(
                Math.toRadians(geo2.latitude)
            )
        val c = 2 * asin(sqrt(a))
        return (R * c)
    }
